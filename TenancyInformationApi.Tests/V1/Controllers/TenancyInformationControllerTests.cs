...
namespace TenancyInformationApi.Tests.V1.Controllers
{
    ...
    public class TenancyInformationControllerTests
    {
        ...
        [Test]
        public void ViewRecordReturnsResponseWithStatus()
        {
            ObjectResult response;

            // 1231 is not a valid tag_ref - should return BadRequest400
            response = _classUnderTest.ViewRecord("1231") as ObjectResult;
            response.Should().NotBeNull();
            response?.StatusCode.Should().Be(400);

            // 123/1 is not in the db - should return NotFound400
            response = _classUnderTest.ViewRecord("123-1") as ObjectResult;
            response.Should().NotBeNull();
